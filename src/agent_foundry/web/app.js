...
async function loadPrograms(){
  ...
  try{
    ...
    $("#program-grid").innerHTML=result.items.map(p=>`<article class="program-card"><div class="program-card-top"><span class="icon-tile ${p.runtime==="python"?"violet":p.status==="ACTIVE"?"blue":"amber"}">${icon(toolIcon(p))}</span>${badge(p)}</div><h3><button class="program-name" data-program="${p.id}">${esc(names[p.name]||p.name)}</button></h3><span class="program-physical">${esc(p.name)}</span><p class="program-description">${esc(p.description)}</p><div class="program-statistics"><span>${p.source==="github"?"공유일":"설치일"} ${dateLabel(p.source==="github"?p.published_at:p.installed_at)}</span><span>추정 절약 ${Number(p.estimated_tokens_saved||0).toLocaleString()} 토큰</span></div><div class="program-card-footer"><span>${p.runtime==="python"?"Python":p.runtime==="http"?"API":"Built-in"} · v${esc(p.version)}${p.requires_db?" · 중앙 DB":""}</span><span>${Number(p.usage_count).toLocaleString()}회 사용 ${icon("chevron")}</span></div></article>`).join("")||'<div class="empty-library">'+icon("search")+'<strong>조건에 맞는 프로그램이 없습니다</strong><p>검색어나 필터를 변경해보세요.</p></div>';
