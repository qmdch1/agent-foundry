...
- 평가 슬롯과 생성·설치 슬롯을 분리해 긴 생성 작업 중에도 다음 요청의 재사용 평가를 진행합니다. 평가에서 발견한 공유 프로그램은 설치 큐로 넘깁니다.
- 테스트 실패 시 직전 소스를 제한된 크기로 전달하고 변경 파일만 다시 받습니다. 합친 결과에도 전체 검증을 적용하며, 생성 단계 기록의 `repair`로 구분합니다.
- 같은 Worker에서 동시에 들어온 Git 동기화 요청은 성공한 한 번의 결과를 공유합니다. 이후 새 요청과 배포 전 확인은 원격을 다시 조회합니다.
- Main의 같은 호출에서 답변과 짧은 생성 명세를 함께 받습니다. Worker가 이 자료를 받아 평가하며 재해석을 줄입니다.
- 비교·집계·저장은 [공통 템플릿](BUILD_TEMPLATES.md)을 주입하고 모델은 요청별 연결 코드와 테스트를 만듭니다.
- 설치된 ACTIVE Python 프로그램은 기존 입력·출력 계약과 권한을 유지하는 범위에서 자동 확장합니다. 이전 테스트와 예시를 다시 검증하고 생성·수정 토큰을 누적합니다.
- 동일 소스·의존성·기본 이미지의 실행 이미지를 재사용합니다. 소스가 달라도 동일 의존성 레이어는 Docker 캐시를 활용합니다.
- 성공한 검증은 이미지·검증기·실행 정책이 같고 유효기간 내일 때 재사용합니다. DB migration과 실제 연결 확인은 배포할 때마다 수행합니다.
- 관리자 프로그램 화면에 최근 7일 단계별 평균 시간·성공·캐시 재사용 횟수를 표시합니다. 실제 개선 폭은 연결한 모델과 프로그램별 기록으로 확인합니다.

외부 AI가 먼저 답변한 자료는 `POST /v1/build-reviews`로 전달할 수 있습니다. 이 API는 검토 작업만 큐에 넣으며 모든 채팅을 자동 감시하지 않습니다. [배포 및 연결 방법](DEPLOYMENT.md)을 참고하세요.
...
- **프로그램**: 실제 Registry의 프로그램을 검색하고 사용 가능·설정 대기·내부 기능으로 구분합니다.
  ‘이 서버 프로그램’과 ‘GitHub 공유 프로그램’을 구분합니다. 설치일, 호출 횟수, 절약 토큰,
...
않고 재사용하거나 호환 확장을 평가합니다. 평가된 ACTIVE Python 도구의 기존 계약·권한·테스트를 유지하는 자동 확장을 지원합니다. 인터페이스 변경은 포함하지
...
완전한 DAG 엔진, Go 생성, 호환되지 않는 도구 변경, 실제 DB 백업 서비스와 스케줄링은 향후 범위입니다.
