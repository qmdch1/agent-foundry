...
사용자 요청에 따라 이 세션에서 만든 CSV 통계·기록 저장·HRMS는 Python 앱·테스트의 UTF-8 바이트 수 ÷ 4 올림값을 초기 환산 기준으로 사용합니다. manifest의 generation_tokens_estimated=true로 구분하고 생성 약 N 토큰으로 표시하며, 실제 세션 사용량을 뜻하지 않습니다. 설치·복구·GitHub 목록 동기화 시 해당 commit의 파일을 DB에 반영하므로 다른 서버에도 최종값이 전달됩니다.
